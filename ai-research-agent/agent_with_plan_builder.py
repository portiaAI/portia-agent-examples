...
def check_emails_exist(emails_content) -> bool:
      ...
      print("Type:", type(emails_content))
      print("First 200 chars:", repr(emails_content[:200]))

      # See if it's JSON string or not.
      if isinstance(emails_content, str):
          try:
              ...
              if isinstance(parsed, list):
                  print("JSON contains list and lengtth is:", len(parsed))
                  return len(parsed) > 0
          ...
          print("Already a list with length:", len(emails_content))
...
def run_agent() -> ResearchAgentOutput:
    ...
    plan = (
        ...
        # Sttep to debug the gmail output, check if its  a list or a str.
        .function_step(
            step_name="debug_gmail_output",
            function=lambda emails: print(f"DEBUG - Gmail output type: {type(emails)}, value: {repr(emails)[:500]}...") or emails,
            args={"emails": StepOutput("fetch_ai_emails")},
        )
