...
print(
    "1. Outreach messages doc: This should contain any messages that you want the agent to parse "
    "and create connections for the LinkedIn profiles within it."
)
print(
    "2. Outreach criteria doc: This should contain the criteria that the agent will use to assess "
    "the LinkedIn profiles."
)
print(
    "The document ID of the Google docs can be found in the URL of the document after the "
    "'document/d/' part."
)
...
for url in linkedin_urls:
    plan_run = portia.run(
        read_task_from_file("research_and_connect.txt")
        .replace("{linked_in_profile_url}", url)
        .replace("{outreach_criteria_id}", outreach_criteria_doc_id)
    )
