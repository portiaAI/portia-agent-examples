...
class EventRegistrationAgent:
    ...
    def _build_plan(self):
        builder = (
            ...
            .single_tool_agent_step(tool="browser_tool",task=("For each event in events, open the event page and attempt registration.\n"
                ...
                "- If a form appears, fill it using details from config (name/email/etc.).\n"
                "- If form requires more than what you have in config try to make up reasonable answers"
                "- If asked to subscribe to a newsletter, only accept if there is no option to decline"
    ...
    def run(self, config, events: EventsSchema) -> AttendanceSchema:
        plan = self._build_plan()
        plan_run = self.portia.run_plan(plan, plan_run_inputs={"events": events, "config": config})
